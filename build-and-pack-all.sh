...
build_gc() {
    ...
    OUTPUT="$(pwd)/dist/${goos}/${goarch}/msak-server"
    if [[ "${goos}" == "windows" ]]; then
        OUTPUT="$(pwd)/dist/${goos}/${goarch}/msak-server.exe"
    fi

    pushd src/cmd/msak-server
...
package() {
    ...
    if [[ "${goos}" == "linux" ]] || [[ "${goos}" == "android" ]]; then
        tar -czf "../../msak-server-${BUILD_NAME}-${goos}-${goarch}.tar.gz" -- *
    else
        zip -r "../../msak-server-${BUILD_NAME}-${goos}-${goarch}.zip" -- *
    fi
    popd
}
